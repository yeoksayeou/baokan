...
function setupNavigation(prevParams, nextParams) {
  prevTarget = prevParams; nextTarget = nextParams;
}
function navigatePrev() { if (prevTarget) window.location = createUrl('index.html', prevTarget); }
function navigateNext() { if (nextTarget) window.location = createUrl('index.html', nextTarget); }
...
function displayYearList() {
  ...
  let html = `<h1>Available Years</h1><div class="year-grid">`;
  years.forEach(y => {
...
function displayMonthDayList(year) {
  ...
  Object.keys(data).sort().forEach(mon => {
    ...
    const monthName = new Date(year,mi,1).toLocaleString('default',{month:'long'});
    const daysInMonth = new Date(year,mi+1,0).getDate();
    const avail = new Set(data[mon].map(o=>o.day));
    const firstDow = new Date(year,mi,1).getDay();

    html += `<div class="month-block"><h3>${monthName}</h3><div class="calendar">`;
    ['Sun','Mon','Tue','Wed','Thu','Fri','Sat']
      .forEach(d=> html+=`<div class="calendar-header">${d}</div>`);
    for(let i=0;i<firstDow;i++) html+=`<div class="calendar-empty"></div>`;
  ...
  html += `<div class="navigation-controls"><div class="nav-pagination">`
        + (prevY
